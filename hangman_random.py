...
def game():
    ...
    f = open("words.txt", "r") # Opens words.txt, a file containing every word in the english vocabulary, written only using alpha letters. (a-z)
    ...
    while True:
        try:
            maxStrikes = int(input('Please input the maximum amount of strikes (0 for none): ')) # Maximum amount of wrong guesses before you take the L.
            break
        except:
            print("Invalid value.")
    # If maxStrikes is chosen to be 0, it "removes" the limit by making the value really big. 200IQ programming right there, folks.
    if maxStrikes == 0:
        maxStrikes = 999999
    ...
    while answer != gameword:
        ...
        if not re.match("^[a-z]*$", letter.lower()):
